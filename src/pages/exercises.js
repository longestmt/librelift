...
export async function renderExercisesPage(container) {
  ...
  `;
    ...
    grid.addEventListener('click', async (e) => {
        ...
        const safeMediaUrl = (ex.mediaUrl && (ex.mediaUrl.startsWith('http://') || ex.mediaUrl.startsWith('https://')))
            ? escapeHTML(String(ex.mediaUrl))
            : null;
      ...
      ${safeMediaUrl ? `
        <a href="${safeMediaUrl}" target="_blank" rel="noopener" class="btn btn-secondary btn-full" style="margin-bottom:var(--sp-4)">
