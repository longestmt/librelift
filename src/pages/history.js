      ...
      <div class="card-header">
        <div>
          ...
          <div class="flex gap-2" style="margin-top:2px">
            <span class="text-xs text-muted">${escapeHTML(w.date || '')}</span>
...
function showWorkoutDetail(workout, sets, onDelete) {
  ...
  body.innerHTML = `
    <div class="text-xs text-muted" style="margin-bottom:var(--sp-3)">${escapeHTML(workout.date || '')}${workout.planName ? ` • ${escapeHTML(workout.planName)}` : ''}${durDisplay ? ` • <span id="dur-display" style="cursor:pointer;text-decoration:underline dotted;text-underline-offset:2px" title="Tap to edit">${durDisplay}</span>` : ''}</div>
