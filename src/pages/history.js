...
function showWorkoutDetail(workout, sets, onDelete) {
  ...
  body.innerHTML = `
    <div class="text-xs text-muted" style="margin-bottom:var(--sp-3)">${workout.date}${workout.planName ? ` • ${escapeHTML(workout.planName)}` : ''}${workout.durationSec ? ` • ${Math.floor(workout.durationSec / 60)}m` : ''}</div>
