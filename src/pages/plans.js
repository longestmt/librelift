    ...
    <div class="card card-clickable" data-plan-id="${plan.id}">
      <div class="card-header">
        ...
        <div class="flex gap-2">
          <button class="btn btn-icon btn-ghost delete-plan-btn" aria-label="Delete ${escapeHTML(plan.name)}" title="Delete">
            <svg aria-hidden="true" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--danger)" stroke-width="2">
    ...
    return `
        <div>
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:var(--sp-2)">
            ...
            <button class="btn btn-ghost btn-icon" aria-label="Start ${escapeHTML(day.name)}" data-start-day="${di}" title="Start ${escapeHTML(day.name)}" style="width:32px;height:32px;color:var(--accent)">
              <svg aria-hidden="true" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="5 3 19 12 5 21 5 3"/></svg>
            </button>
          </div>
    ...
    daysContainer.innerHTML = planDays.map((day, di) => `
      ...
      return `
              <div class="flex items-center gap-2 text-sm">
                ...
                <button class="btn btn-ghost btn-icon" aria-label="Remove ${safeExName}" title="Remove ${safeExName}" data-remove-ex="${di}-${ei}" style="width:28px;height:28px"><span aria-hidden="true">×</span></button>
              </div>
            `;
    }).join('')}
        </div>
