...
async function showExercisePicker(exercises, exContainer, unit) {
  ...
  body.innerHTML = `<div class="search-bar" style="margin-bottom:var(--sp-3)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg><input class="input" id="add-ex-search" placeholder="Search..."/></div><div id="add-ex-list" style="max-height:300px;overflow-y:auto" class="flex flex-col gap-1">${exercises.map(ex => `<button class="list-item" data-id="${ex.id}" data-name="${escapeHTML(String(ex.name || ''))}" style="width:100%;border:none;background:none;text-align:left;font-family:var(--font-sans);color:var(--text-primary)"><span style="flex:1"><div class="text-sm font-medium">${escapeHTML(String(ex.name || ''))}</div><div class="text-xs text-muted">${escapeHTML(String(ex.muscleGroup || ''))} • ${escapeHTML(String(ex.equipment || ''))}</div></span></button>`).join('')}</div>`;
