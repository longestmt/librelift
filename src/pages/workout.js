    ...
    return `<div class="card" data-ei="${ei}">
      <div class="card-header" style="cursor:pointer" data-toggle="${ei}">
        ...
        <div class="flex items-center gap-1">
          <button class="btn btn-ghost btn-icon" data-show-history="${ei}" title="History" aria-label="History for ${ex.exerciseName}" style="width:32px;height:32px"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></button>
          <button class="btn btn-ghost btn-icon" data-swap-ex="${ei}" title="Swap exercise" aria-label="Swap ${ex.exerciseName}" style="width:32px;height:32px"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M7 16V4m0 0L3 8m4-4l4 4"/><path d="M17 8v12m0 0l4-4m-4 4l-4-4"/></svg></button>
          <button class="btn btn-ghost btn-icon" data-show-plates="${ei}" title="Plates" aria-label="Plate calculator for ${ex.exerciseName}" style="width:32px;height:32px"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="6" width="4" height="12" rx="1"/><rect x="18" y="6" width="4" height="12" rx="1"/><line x1="6" y1="12" x2="18" y2="12"/></svg></button>
        </div>
      </div>
...
function renderSetRow(set, si, ei, exerciseName) {
  ...
  return `<div class="set-row">
    ...
    <input class="input-inline ${lockedCls}" type="number" aria-label="Weight for ${exerciseName} set ${set.setNumber}" value="${set.weight}" data-ei="${ei}" data-si="${si}" data-field="weight" inputmode="decimal" ${locked}/>
    <input class="input-inline ${lockedCls}" type="number" aria-label="Reps for ${exerciseName} set ${set.setNumber}" value="${set.reps}" data-ei="${ei}" data-si="${si}" data-field="reps" inputmode="numeric" ${locked}/>
    <input class="input-inline ${lockedCls}" type="number" aria-label="RPE for ${exerciseName} set ${set.setNumber}" value="${set.rpe || ''}" data-ei="${ei}" data-si="${si}" data-field="rpe" inputmode="decimal" placeholder="—" ${locked}/>
    <button class="set-check ${cls}" aria-label="Mark ${exerciseName} set ${set.setNumber} complete" aria-pressed="${isPressed}" data-ei="${ei}" data-si="${si}">${icon}</button>
  </div>`;
}
