...
function renderExerciseCard(ex, ei, unit) {
  ...
  const safeName = escapeHTML(ex.exerciseName);
  ...
  return `<div class="card" data-ei="${ei}" style="${ex.supersetGroup ? 'border:none;box-shadow:none;background:transparent;padding:0' : ''}">
      <div class="card-header" style="cursor:pointer" data-toggle="${ei}">
        ...
        <div class="flex items-center gap-1">
          <button class="btn btn-ghost btn-icon" data-show-history="${ei}" aria-label="History for ${safeName}" title="History" style="width:32px;height:32px"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></button>
          <button class="btn btn-ghost btn-icon" data-swap-ex="${ei}" aria-label="Swap ${safeName}" title="Swap exercise" style="width:32px;height:32px"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M7 16V4m0 0L3 8m4-4l4 4"/><path d="M17 8v12m0 0l4-4m-4 4l-4-4"/></svg></button>
          <button class="btn btn-ghost btn-icon" data-show-rm="${ei}" aria-label="1RM Calculator for ${safeName}" title="1RM Calculator" style="width:32px;height:32px"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v4m0 12v4M2 12h4m12 0h4"/><circle cx="12" cy="12" r="3"/></svg></button>
          <button class="btn btn-ghost btn-icon" data-show-plates="${ei}" aria-label="Plate Calculator for ${safeName}" title="Plates" style="width:32px;height:32px"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="6" width="4" height="12" rx="1"/><rect x="18" y="6" width="4" height="12" rx="1"/><line x1="6" y1="12" x2="18" y2="12"/></svg></button>
        </div>
      </div>
      <div class="exercise-body" style="${ex.collapsed ? 'display:none' : ''}">
        ...
        ${ex.sets.map((set, si) => renderSetRow(set, si, ei, safeName)).join('')}
        <div class="flex gap-2" style="margin-top:var(--sp-2)"><button class="btn btn-ghost text-sm" data-add-set="${ei}" aria-label="Add set to ${safeName}" style="flex:1">+ Set</button>${ex.sets.length > 1 ? `<button class="btn btn-ghost text-sm text-danger" data-remove-set="${ei}" aria-label="Remove last set from ${safeName}">− Set</button>` : ''}<button class="btn btn-ghost text-sm" data-ex-note="${ei}" aria-label="Add note to ${safeName}" title="Note"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 1 1 3 3L7 19l-4 1 1-4 12.5-12.5z"/></svg></button></div>
...
function renderSetRow(set, si, ei, safeName) {
  ...
  return `<div class="set-row">
    ...
    <input class="input-inline ${lockedCls}" type="number" aria-label="Weight for set ${set.setNumber} of ${safeName}" value="${set.weight}" data-ei="${ei}" data-si="${si}" data-field="weight" inputmode="decimal" ${locked}/>
    <input class="input-inline ${lockedCls}" type="number" aria-label="Reps for set ${set.setNumber} of ${safeName}" value="${set.reps}" data-ei="${ei}" data-si="${si}" data-field="reps" inputmode="numeric" ${locked}/>
    <input class="input-inline ${lockedCls}" type="number" aria-label="RPE for set ${set.setNumber} of ${safeName}" value="${set.rpe || ''}" data-ei="${ei}" data-si="${si}" data-field="rpe" inputmode="decimal" placeholder="—" ${locked}/>
    <button class="set-check ${cls}" aria-label="Mark set ${set.setNumber} of ${safeName} complete" aria-pressed="${isPressed}" data-ei="${ei}" data-si="${si}">${icon}</button>
  </div>`;
}
