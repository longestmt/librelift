  ...
  return `<div class="card" data-ei="${ei}" style="${ex.supersetGroup ? 'border:none;box-shadow:none;background:transparent;padding:0' : ''}">
      <div class="card-header" style="cursor:pointer" data-toggle="${ei}">
        ...
        <div class="flex items-center gap-1">
          <button class="btn btn-ghost btn-icon" data-show-history="${ei}" aria-label="History for ${escapeHTML(ex.exerciseName)}" title="History for ${escapeHTML(ex.exerciseName)}" style="width:32px;height:32px"><svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></button>
          <button class="btn btn-ghost btn-icon" data-swap-ex="${ei}" aria-label="Swap ${escapeHTML(ex.exerciseName)}" title="Swap ${escapeHTML(ex.exerciseName)}" style="width:32px;height:32px"><svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M7 16V4m0 0L3 8m4-4l4 4"/><path d="M17 8v12m0 0l4-4m-4 4l-4-4"/></svg></button>
          <button class="btn btn-ghost btn-icon" data-show-rm="${ei}" aria-label="1RM Calculator for ${escapeHTML(ex.exerciseName)}" title="1RM Calculator for ${escapeHTML(ex.exerciseName)}" style="width:32px;height:32px"><svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v4m0 12v4M2 12h4m12 0h4"/><circle cx="12" cy="12" r="3"/></svg></button>
          <button class="btn btn-ghost btn-icon" data-show-plates="${ei}" aria-label="Plates for ${escapeHTML(ex.exerciseName)}" title="Plates for ${escapeHTML(ex.exerciseName)}" style="width:32px;height:32px"><svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="6" width="4" height="12" rx="1"/><rect x="18" y="6" width="4" height="12" rx="1"/><line x1="6" y1="12" x2="18" y2="12"/></svg></button>
        </div>
      </div>
      <div class="exercise-body" style="${ex.collapsed ? 'display:none' : ''}">
        ...
        <div class="flex gap-2" style="margin-top:var(--sp-2)"><button class="btn btn-ghost text-sm" data-add-set="${ei}" aria-label="Add set to ${escapeHTML(ex.exerciseName)}" style="flex:1">+ Set</button>${ex.sets.length > 1 ? `<button class="btn btn-ghost text-sm text-danger" data-remove-set="${ei}" aria-label="Remove set from ${escapeHTML(ex.exerciseName)}">− Set</button>` : ''}<button class="btn btn-ghost text-sm" data-ex-note="${ei}" aria-label="Notes for ${escapeHTML(ex.exerciseName)}" title="Notes for ${escapeHTML(ex.exerciseName)}"><svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 1 1 3 3L7 19l-4 1 1-4 12.5-12.5z"/></svg></button></div>
...
function renderLinkButton(ei) {
  return `<div class="superset-link-zone" style="display:flex;justify-content:center;padding:var(--sp-1) 0">
    <button class="btn btn-ghost text-xs" data-link-superset="${ei}" aria-label="Link as superset" style="padding:2px var(--sp-2);color:var(--text-muted);opacity:0.6" title="Link as superset">
      <svg aria-hidden="true" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>
      Link
    </button>
  </div>`;
}
