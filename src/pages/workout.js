  ...
  return `<div class="card" data-ei="${ei}" style="${ex.supersetGroup ? 'border:none;box-shadow:none;background:transparent;padding:0' : ''}">
      <div class="card-header" style="cursor:pointer" data-toggle="${ei}">
        ...
        <div class="flex items-center gap-1">
          <button class="btn btn-ghost btn-icon" data-show-history="${ei}" title="History" aria-label="History for ${escapeHTML(ex.exerciseName)}" style="width:32px;height:32px"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></button>
          <button class="btn btn-ghost btn-icon" data-swap-ex="${ei}" title="Swap exercise" aria-label="Swap ${escapeHTML(ex.exerciseName)}" style="width:32px;height:32px"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M7 16V4m0 0L3 8m4-4l4 4"/><path d="M17 8v12m0 0l4-4m-4 4l-4-4"/></svg></button>
          <button class="btn btn-ghost btn-icon" data-show-rm="${ei}" title="1RM Calculator" aria-label="1RM Calculator for ${escapeHTML(ex.exerciseName)}" style="width:32px;height:32px"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v4m0 12v4M2 12h4m12 0h4"/><circle cx="12" cy="12" r="3"/></svg></button>
      ...
      <div class="exercise-body" style="${ex.collapsed ? 'display:none' : ''}">
        ...
        <div class="flex gap-2" style="margin-top:var(--sp-2)"><button class="btn btn-ghost text-sm" data-add-set="${ei}" style="flex:1" aria-label="Add set to ${escapeHTML(ex.exerciseName)}">+ Set</button>${ex.sets.length > 1 ? `<button class="btn btn-ghost text-sm text-danger" data-remove-set="${ei}" aria-label="Remove set from ${escapeHTML(ex.exerciseName)}">− Set</button>` : ''}<button class="btn btn-ghost text-sm" data-ex-note="${ei}" title="Note" aria-label="Notes for ${escapeHTML(ex.exerciseName)}"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 1 1 3 3L7 19l-4 1 1-4 12.5-12.5z"/></svg></button></div>
...
function renderLinkButton(ei) {
  return `<div class="superset-link-zone" style="display:flex;justify-content:center;padding:var(--sp-1) 0">
    <button class="btn btn-ghost text-xs" data-link-superset="${ei}" style="padding:2px var(--sp-2);color:var(--text-muted);opacity:0.6" aria-label="Link ${escapeHTML(activeWorkout.exercises[ei].exerciseName)} as superset" title="Link as superset">
    ...
    if (ex.supersetGroup) {
      ...
      html += `<div class="superset-wrapper">
        <div class="superset-header">
          ...
          <button class="btn btn-ghost text-xs" data-unlink-superset="${groupStart}" style="margin-left:auto;padding:2px var(--sp-2);color:var(--text-muted)" aria-label="Unlink superset ${escapeHTML(names)}">Unlink</button>
        </div>
