...
function renderExerciseCard(ex, ei, unit) {
  ...
  return `<div class="card" data-ei="${ei}" style="${ex.supersetGroup ? 'border:none;box-shadow:none;background:transparent;padding:0' : ''}">
      ...
      <div class="exercise-body" style="${ex.collapsed ? 'display:none' : ''}">
        ...
        <div class="flex gap-2" style="margin-top:var(--sp-2)"><button class="btn btn-ghost text-sm" aria-label="Add set for ${escapeHTML(ex.exerciseName)}" data-add-set="${ei}" style="flex:1">+ Set</button>${ex.sets.length > 1 ? `<button class="btn btn-ghost text-sm text-danger" aria-label="Remove set for ${escapeHTML(ex.exerciseName)}" data-remove-set="${ei}">− Set</button>` : ''}<button class="btn btn-ghost text-sm" aria-label="Note for ${escapeHTML(ex.exerciseName)}" data-ex-note="${ei}" title="Note"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 1 1 3 3L7 19l-4 1 1-4 12.5-12.5z"/></svg></button></div>
