...
export async function renderAnalyticsPage(container) {
  ...
  if (bwEntries.length > 0) {
    ...
    bwArea.innerHTML = `<div class="flex items-center justify-between" style="margin-bottom:var(--sp-2)"><span class="font-bold text-accent" style="font-size:var(--text-lg)">${latest.value} ${escapeHTML(String(latest.unit || unit))}</span><span class="text-xs text-muted">${sorted.length} entries</span></div>`;
