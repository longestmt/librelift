...
export function showPRToast(exerciseName, pr, unit) {
    ...
    const safePrLabel = escapeHTML(String(pr.label));
    const safeUnit = escapeHTML(String(unit));
    const safePrPrev = pr.prev ? escapeHTML(String(pr.prev)) : null;
    const detail = safePrPrev ? `Previous best: ${safePrPrev} ${safeUnit}` : '';
    toast.innerHTML = `
    <span class="pr-icon">🏆</span>
    <span class="pr-text">
      <span class="pr-title">Personal Record!</span>
      <span class="pr-detail">${safeExerciseName} — ${safePrLabel} ${safeUnit}${detail ? ` (was ${safePrPrev} ${safeUnit})` : ''}</span>
    </span>`;
    c.appendChild(toast);
